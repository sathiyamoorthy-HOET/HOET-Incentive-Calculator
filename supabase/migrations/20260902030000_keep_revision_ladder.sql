-- Leaves the revision ladder alone when a caller does not mention it.
--
-- set_config replaces the whole config in one transaction, which is the right
-- shape for a document the UI edits as a whole -- but it made every absent key
-- mean "delete". The deployed app sends no 'revPen' until its own release goes
-- out, and any browser tab left open on the older build never will, so a
-- single save from one of those would have cleared what revisions cost.
--
-- Absent now means unchanged. An explicit empty list still clears the ladder,
-- which is how the rate card removes every rung.
