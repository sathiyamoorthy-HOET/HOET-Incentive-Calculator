-- Prices reviewing by video type, not by one rate for everything.
--
-- Reviewing a minute of a motion-graphics cut is not the same job as reviewing
-- a minute of a screen recording, so the rate moves onto the video type beside
-- the editing rates it will be compared against. It still does not vary by
-- slab: the work is the same whoever does it.
--
-- Every type inherits whatever the single rate was, so nobody's pay changes on
-- the way through. The old column goes, and a caller that does not mention a
-- type's review rate leaves it alone -- an older build sends no 'review' key,
-- and one save from a stale browser tab would otherwise zero every one.

alter table public.video_categories add column if not exists review_rate numeric not null default 0
  check (review_rate >= 0);

update public.video_categories c
   set review_rate = s.review_rate
  from public.app_settings s
 where s.id and c.review_rate = 0;

alter table public.app_settings drop column if exists review_rate;
